...
const Pants = (props) => {
	...
	return (
		<InfoWrapper>
			<ListItems>
				{products.filter((product) => product.title.includes(pant.toUpperCase())).map((filteredItem, i) => (
					<Item key={i}>
				...
				{products.filter((product) => product.title.includes(trousers.toUpperCase())).map((filteredItem, i) => (
					<Item key={i}>
						<ItemLink to={`/product/${filteredItem.title_url}`}>
							<ItemImage src={filteredItem.img_url1} /> <ItemTitle> {filteredItem.title} </ItemTitle>{' '}
							<ItemPrice>
